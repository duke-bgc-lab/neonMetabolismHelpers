...
model_neon_metab_mle <- function(input_dir = 'data/sm_input/',
  ...
  for(j in 1:length(input_ts)){
    ...
    if(sum(is.na(input_dat$DO.obs)) == nrow(input_dat)){
      ...
      cat(paste0(logcode, '-- ',site,'-' ,year, ' Has data'),
          append = TRUE, file = 'nmh_log.txt', sep = '\n')
    }
    ...
    # write the fit data
    fn_prefix <- paste0(write_dir, 'MLE/dat_fit/', site, '_', wyear, '_Q-', q_type, '_z-', z_method, '_TS-', sensor_src, '_')
    readr::write_csv(dat_fit, paste0(fn_prefix, 'dat_fit.csv'))
    
    # write the specs
    specs_out <- data.frame(unlist(get_specs(dat_metab)))
    fn_prefix <- paste0(write_dir, 'MLE/specs/', site, '_', wyear, '_Q-', q_type, '_z-', z_method, '_TS-', sensor_src, '_')
    readr::write_csv(specs_out, paste0(fn_prefix, 'specs.csv'))
    
    # write the output data
    data_out = get_data(dat_metab)
    fn_prefix <- paste0(write_dir, 'MLE/mod_obs_DO/', site, '_', wyear,'_Q-', q_type, '_z-', z_method, '_TS-', sensor_src, '_')
