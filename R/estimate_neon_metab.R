estimate_neon_metab <- function(mod_dir = 'data/model_runs/',
    ...
    for(i in 1:n_mods) {
        ...
        site <- str_split(daily, c('_','/'))[[2]][6] %>%
            substr(.,1,4)

        start_date <- str_split(daily, '_')[[1]][3]
        end_date <- str_split(daily, '_')[[1]][4]
        ...
        if(!'date' %in% names(DO)){
            DO <- DO %>%
                mutate(date = lubridate::date(solar.time))
        }
    ...
    estimates <- map_dfr(list.files(write_dir,
                                    full.names = TRUE),
                         read_csv)
