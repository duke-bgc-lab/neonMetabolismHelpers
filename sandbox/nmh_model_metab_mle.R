...
model_neon_metab_mle <- function(input_dir = 'data/sm_input/',
  ...
  if(!type %in% c('raw','source', 'simulated')){
  ...
  for(j in 1:length(site_years$site)){
    ...
    input_dat <- try(readr::read_csv(glue::glue(input_dir,'{type}/{site}_{type}_smReady.csv')))
    ...
    start_date <- dplyr::first(lubridate::date(input_dat_sub$solar.time))
    end_date <- dplyr::last(lubridate::date(input_dat_sub$solar.time))
    ...
    write_dir = glue::glue('data/model_runs/{type}/')
